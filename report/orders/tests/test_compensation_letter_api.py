# tests/test_compensation_letter_api.py
import pytest
import json
from unittest.mock import Mock, patch, MagicMock, ANY
from django.http import JsonResponse, HttpResponse
from django.contrib.auth import get_user_model
from orders.compensation_letter_api import generate_compensation_letter_api

User = get_user_model()


class TestCompensationLetterAPI:
    """Tests pour la vue generate_compensation_letter_api"""

    @pytest.fixture
    def user(self, db):
        return User.objects.create_user(
            email='test@example.com',
            password='testpass',
            first_name='Test',
            last_name='User'
        )

    @pytest.fixture
    def mock_request(self, user):
        request = Mock()
        request.method = 'GET'
        request.user = user
        return request

    @pytest.mark.django_db
    def test_method_not_allowed(self, mock_request):
        """Test avec une méthode non autorisée"""
        mock_request.method = 'PUT'
        response = generate_compensation_letter_api(mock_request, 1)
        
        assert isinstance(response, JsonResponse)
        assert response.status_code == 405
        import json
        data = json.loads(response.content)
        assert data['success'] is False
        assert 'Méthode non autorisée' in data['error']

    @pytest.mark.django_db
    @patch('orders.compensation_letter_api.get_object_or_404')
    def test_bon_commande_not_found(self, mock_get_object, mock_request):
        """Test quand le bon de commande n'existe pas"""
        mock_get_object.side_effect = Exception('Not found')
        
        response = generate_compensation_letter_api(mock_request, 999)
        
        assert isinstance(response, JsonResponse)
        assert response.status_code == 404
        import json
        data = json.loads(response.content)
        assert data['success'] is False
        assert 'Bon de commande non trouvé' in data['error']

    @pytest.mark.django_db
    @patch('orders.compensation_letter_api.get_object_or_404')
    @patch('orders.compensation_letter_api.collect_penalty_context')
    @patch('orders.compensation_letter_api.generate_compensation_letter')
    @patch('orders.compensation_letter_api.threading.Thread')
    def test_successful_pdf_generation(
        self, mock_thread, mock_generate_pdf, mock_collect_context, 
        mock_get_object, mock_request
    ):
        """Test la génération réussie du PDF"""
        # Setup mocks
        bon_commande = Mock()
        bon_commande.id = 1
        bon_commande.numero = 'TEST123'
        mock_get_object.return_value = bon_commande
        mock_collect_context.return_value = {
            'po_number': 'TEST123',
            'supplier': 'Test Supplier'
        }
        
        mock_pdf_buffer = Mock()
        mock_pdf_buffer.read.return_value = b'%PDF-1.4 fake pdf content'
        mock_generate_pdf.return_value = mock_pdf_buffer
        
        mock_thread_instance = Mock()
        mock_thread.return_value = mock_thread_instance

        # Call the view
        response = generate_compensation_letter_api(mock_request, 1)

        # Assertions
        assert isinstance(response, HttpResponse)
        assert response.status_code == 200
        assert response['content-type'] == 'application/pdf'
        assert 'Lettre_Compensation_TEST123.pdf' in response['content-disposition']
        assert b'%PDF-1.4 fake pdf content' in response.content

        # Verify mocks were called
        mock_get_object.assert_called_once_with(ANY, id=1)
        mock_collect_context.assert_called_once_with(bon_commande)
        mock_generate_pdf.assert_called_once()
        mock_thread.assert_called_once()

    @pytest.mark.django_db
    @patch('orders.compensation_letter_api.get_object_or_404')
    @patch('orders.compensation_letter_api.collect_penalty_context')
    def test_generation_error(self, mock_collect_context, mock_get_object, mock_request):
        """Test quand une erreur se produit pendant la génération"""
        mock_get_object.return_value = Mock()
        mock_collect_context.side_effect = Exception('Test error')

        response = generate_compensation_letter_api(mock_request, 1)

        assert isinstance(response, JsonResponse)
        assert response.status_code == 500
        import json
        data = json.loads(response.content)
        assert data['success'] is False
        assert 'Erreur lors de la génération' in data['error']

    @pytest.mark.django_db
    @patch('orders.compensation_letter_api.get_object_or_404')
    @patch('orders.compensation_letter_api.collect_penalty_context')
    @patch('orders.compensation_letter_api.generate_compensation_letter')
    def test_email_sent_async(self, mock_generate_pdf, mock_collect_context, mock_get_object, mock_request):
        """Test que l'email est envoyé de façon asynchrone"""
        mock_get_object.return_value = Mock()
        mock_collect_context.return_value = {}
        mock_pdf_buffer = Mock()
        mock_generate_pdf.return_value = mock_pdf_buffer

        with patch('orders.compensation_letter_api.send_penalty_notification') as mock_send_email:
            with patch('orders.compensation_letter_api.threading.Thread') as mock_thread:
                mock_thread_instance = Mock()
                mock_thread.return_value = mock_thread_instance

                response = generate_compensation_letter_api(mock_request, 1)

                # Verify thread was started
                mock_thread.assert_called_once()
                mock_thread_instance.start.assert_called_once()

    def test_post_method_allowed(self, mock_request):
        """Test que la méthode POST est autorisée"""
        mock_request.method = 'POST'
        
        with patch('orders.compensation_letter_api.get_object_or_404') as mock_get_object:
            mock_get_object.return_value = Mock()
            with patch('orders.compensation_letter_api.collect_penalty_context'):
                with patch('orders.compensation_letter_api.generate_compensation_letter'):
                    response = generate_compensation_letter_api(mock_request, 1)
                    
                    # Should not return method not allowed
                    assert not isinstance(response, JsonResponse) or response.status_code != 405

    @patch('orders.compensation_letter_api.get_object_or_404')
    @patch('orders.compensation_letter_api.collect_penalty_context')
    @patch('orders.compensation_letter_api.generate_compensation_letter')
    def test_user_email_in_context(self, mock_generate_pdf, mock_collect_context, mock_get_object, mock_request):
        """Test que l'email utilisateur est passé au générateur PDF"""
        mock_get_object.return_value = Mock()
        mock_collect_context.return_value = {}
        mock_pdf_buffer = Mock()
        mock_generate_pdf.return_value = mock_pdf_buffer

        mock_request.user.email = 'test@example.com'

        with patch('orders.compensation_letter_api.threading.Thread'):
            generate_compensation_letter_api(mock_request, 1)
            
            # Verify user email was passed to PDF generator
            call_kwargs = mock_generate_pdf.call_args[1]
            assert call_kwargs['user_email'] == 'test@example.com'